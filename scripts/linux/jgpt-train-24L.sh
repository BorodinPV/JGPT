#!/usr/bin/env bash
# =============================================================
# jgpt-train-24L.sh — обучение ~63M (24 слоя) на RTX 3080 10 GB
#
...
ENV_FILE="$ROOT/env/24L-63M.env"
CKPT_DIR="$ROOT/checkpoints/all_books"
CKPT_BACKUP="$ROOT/checkpoints/all_books_12L_backup"
...
Обучение JGPT ~63M: 24 слоя, d_model=384, seq=1024, batch=1, sampled CE.
Пресет VRAM: ${ENV_FILE}

Options:
  --data-dir PATH   каталог с .txt (по умолчанию: data/books/libru_txt_clean)
  --fresh           архивировать checkpoints/all_books/* в all_books_12L_backup
...
if [[ ! -f "$ENV_FILE" ]]; then
    echo "[24L] ERROR: missing preset: $ENV_FILE" >&2
    exit 1
fi
...
if [[ "$txt_count" -eq 0 ]]; then
    echo "[24L] ERROR: no .txt files in $DATA_DIR" >&2
    exit 1
fi
...
if [[ "$DO_FRESH" -eq 1 ]]; then
    ...
    shopt -u nullglob
    echo "[24L] --fresh: moved $moved file(s) to $CKPT_BACKUP"
fi
...
if [[ "$has_ckpt" -eq 1 ]]; then
    echo "[24L] NOTE: found checkpoint in $CKPT_DIR — resume (must be 24 layers / seq ${JGPT_MAX_SEQ_LEN})"
elif [[ "$DO_FRESH" -eq 0 ]]; then
    echo "[24L] NOTE: training from scratch (no checkpoint). Use --fresh if old 12L weights remain."
fi

echo ""
echo "════════════════════════════════════════════════════════════"
echo " JGPT Train 24L (~63M)  |  $(date '+%Y-%m-%d %H:%M:%S')"
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
    ...
    if [[ -f "$ROOT/build/libjgpt_cuda.so" ]]; then
        ...
        echo "[24L] ERROR: --no-build but no lib in build/" >&2
        exit 1
    fi
fi

# exec-maven-plugin режет -Dexec.args по пробелам — путь «Рабочий стол» ломался.
mvn -q compile
CP_FILE="$(mktemp)"
