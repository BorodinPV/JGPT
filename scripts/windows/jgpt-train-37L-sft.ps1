...
#
# jgpt-train-37L-sft.ps1 - Windows port of jgpt-train-37L-sft.sh
# ~100.4M: 37 layers, vocab 16k, seq 2048, SFT
#
# From scratch: BPE checkpoints\tokenizer_sft_16k.bin and
# checkpoints\sft_37L_16k_2048\. Not jgpt-smart.sh.
#
# Usage:
#   .\scripts\windows\jgpt-train-37L-sft.ps1
#   .\scripts\windows\jgpt-train-37L-sft.ps1 --fresh
#   $env:JGPT_BATCH_SIZE="2"; $env:JGPT_ACCUMULATION_STEPS="64"; .\scripts\windows\jgpt-train-37L-sft.ps1 --no-build
#
# Stop:   Ctrl+C  (shutdown hook writes checkpoint_final.bin)
# Resume: .\scripts\windows\jgpt-train-37L-sft.ps1
#
# Chat (after training):
#   . .\build\jgpt-cuda-env.ps1
#   Get-Content .\env\37L-sft-100M.env
#   java ... com.veles.llm.jgpt.app.InferChat --boo . --layers 37 --seq-len 2048 `
#     --model checkpoints\sft_37L_16k_2048\model_best.bin `
#     --tokenizer checkpoints\tokenizer_sft_16k.bin
$ErrorActionPreference = "Stop"

$Root = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
Set-Location $Root

$LogFile = Join-Path $Root "training_sft_37L.log"
$EnvFile = Join-Path $Root "env\37L-sft-100M.env"
$CkptDir = Join-Path $Root "checkpoints\sft_37L_16k_2048"
$CkptBackup = Join-Path $Root "checkpoints\sft_37L_16k_2048_prev_backup"
$TokenizerFile = Join-Path $Root "checkpoints\tokenizer_sft_16k.bin"

$DataDir = $env:JGPT_DATA_DIR
if (-not $DataDir) { $DataDir = "data\sft\raw" }
$DoFresh = $false
$SkipBuild = $false

function Show-Usage {
    Write-Host @"
Usage: .\scripts\windows\jgpt-train-37L-sft.ps1 [OPTIONS]

Train JGPT ~100.4M SFT: 37 layers, seq=2048, vocab=16000, JSONL
(loss on assistant replies only). Data: data\sft\raw
Preset: env\37L-sft-100M.env
batch=4, accum=32 (effective batch 128), LR=1e-3, sampled CE 384.

Options:
  --data-dir PATH   directory with .jsonl (default: data\sft\raw)
  --fresh           move preset checkpoints and tokenizer_sft_16k.bin aside
    ...
    switch ($argList[$i]) {
        "--data-dir" {
            if ($i + 1 -ge $argList.Count) {
                Write-Host "[37L-SFT] ERROR: --data-dir requires a path" -ForegroundColor Red
                Show-Usage
                exit 1
            }
            $DataDir = $argList[$i + 1]
...
if (-not (Test-Path $EnvFile)) {
    Write-Host "[37L-SFT] ERROR: missing preset: $EnvFile" -ForegroundColor Red
    exit 1
}
...
$exportPy = Join-Path $Root "scripts\sft-export-jsonl.py"
if ($jsonlBefore -gt 0) {
    Write-Host "[37L-SFT] parquet export skipped ($jsonlBefore jsonl already in $DataDir)"
} elseif (Test-Path $exportPy) {
    Write-Host "[37L-SFT] parquet -> jsonl (if missing)..."
    $py = $null
    ...
    if ($py) {
        $null = Get-NativeOutput { & $py $exportPy }
        if ($LASTEXITCODE -ne 0) {
            Write-Host "[37L-SFT] WARN: parquet export skipped (pyarrow not required if jsonl exists)" -ForegroundColor Yellow
        }
    } else {
        Write-Host "[37L-SFT] WARN: python not found, parquet export skipped" -ForegroundColor Yellow
    }
}

$jsonlCount = Get-JsonlCount $DataDir
if ($jsonlCount -eq 0) {
    Write-Host "[37L-SFT] ERROR: no .jsonl files in $DataDir (JGPT_SFT=1)" -ForegroundColor Red
    exit 1
}
...
if (-not $javaHome) {
    Write-Host "[37L-SFT] ERROR: JDK 25+ not found. Set JAVA_HOME, for example:" -ForegroundColor Red
...
if (-not $userFaTile) {
    ...
    if ($cap -eq "7.5") {
        ...
        Write-Host "[37L-SFT] GPU compute $cap (Turing): JGPT_FA_TILE_SIZE=64"
    }
}
...
if ($DoFresh) {
    ...
    if (Test-Path $TokenizerFile) {
        Move-Item -LiteralPath $TokenizerFile -Destination $CkptBackup -Force
        $moved += 1
    }
    Write-Host "[37L-SFT] --fresh: moved $moved file(s) to $CkptBackup"
}
...
if ($hasCkpt) {
    Write-Host "[37L-SFT] NOTE: found checkpoint in $CkptDir - resume (37L / seq $($env:JGPT_MAX_SEQ_LEN) / vocab $($env:JGPT_VOCAB_SIZE))"
} elseif (Test-Path $TokenizerFile) {
    Write-Host "[37L-SFT] NOTE: tokenizer exists, weights from scratch: $TokenizerFile"
} else {
    Write-Host "[37L-SFT] NOTE: training from scratch, will train BPE vocab=$($env:JGPT_VOCAB_SIZE)"
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " JGPT Train 37L SFT (~100.4M)  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $dll)) {
        Write-Host "[37L-SFT] ERROR: --no-build but no lib in build\ ($dll)" -ForegroundColor Red
        exit 1
    }
}
...
$mvn = Find-Mvn
if (-not $mvn) {
    Write-Host "[37L-SFT] ERROR: Maven not found. Install it, then open a NEW PowerShell:" -ForegroundColor Red
...
Write-Host "[37L-SFT] mvn compile..."
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
& $mvn -q compile
...
Write-Host "[37L-SFT] starting AllBooksTrain (resume if checkpoint_final.bin exists)..."
