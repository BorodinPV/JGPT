...
#
# Interactive InferChat for 37L SFT (model_best.bin).
# Does not need mvn on PATH: finds IDEA/bundled Maven, or uses target\classes + .m2 jars.
#
#   .\scripts\windows\jgpt-chat-37L-sft.ps1
#   .\scripts\windows\jgpt-chat-37L-sft.ps1 --prompt "Privet"
...
$ModelDefault = "checkpoints\sft_37L_16k_2048\model_best.bin"
$TokDefault = "checkpoints\tokenizer_sft_16k.bin"
...
if (-not (Test-Path $EnvFile)) {
    Write-Host "[37L-CHAT] ERROR: missing $EnvFile" -ForegroundColor Red
    exit 1
}
...
if (-not $javaHome) {
    Write-Host "[37L-CHAT] ERROR: JDK 25+ not found. Set JAVA_HOME." -ForegroundColor Red
    exit 1
}
...
if (-not $env:JGPT_CUDA_LIB -or -not (Test-Path $env:JGPT_CUDA_LIB)) {
    Write-Host "[37L-CHAT] ERROR: no CUDA JNI. Expected $dll" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $modelAbs)) {
    Write-Host "[37L-CHAT] ERROR: no weights: $modelAbs" -ForegroundColor Red
    exit 1
}
if (-not (Test-Path $tokAbs)) {
    Write-Host "[37L-CHAT] ERROR: no tokenizer: $tokAbs" -ForegroundColor Red
    exit 1
}
...
if ($mvn) {
    ...
    Write-Host "[37L-CHAT] mvn not on PATH; using target\classes + .m2 jars"
    $cp = Get-FallbackClasspath $Root
    if (-not $cp) {
        Write-Host "[37L-CHAT] ERROR: no Maven and no compiled classes/jars." -ForegroundColor Red
...
Write-Host "[37L-CHAT] model=$ModelDefault  (empty line / quit = exit)"
& $javaExe @javaArgs
exit $LASTEXITCODE
