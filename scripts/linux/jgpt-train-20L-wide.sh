#!/usr/bin/env bash
# =============================================================
# jgpt-train-20L-wide.sh — books LM pretrain, 20L / d=512 / seq 1024
# Checkpoints: checkpoints/wide_20L_16k_1024/  Tokenizer: tokenizer_wide_16k.bin
# Does NOT touch sft_37L_*.
#
# Usage:
#   ./scripts/linux/jgpt-train-20L-wide.sh --no-build
# Windows: .\scripts\windows\jgpt-train-20L-wide.cmd --no-build
# =============================================================
set -euo pipefail
...
LOG_FILE="$ROOT/training_20L_wide.log"
ENV_FILE="$ROOT/env/20L-wide-pretrain.env"
CKPT_DIR="$ROOT/checkpoints/wide_20L_16k_1024"
CKPT_BACKUP="$ROOT/checkpoints/wide_20L_16k_1024_prev_backup"
...
Books LM pretrain (20L, d_model=512, seq=1024, vocab=16000).
Preset: ${ENV_FILE}
Checkpoints: checkpoints/wide_20L_16k_1024

Options:
  --data-dir PATH   каталог с .txt (по умолчанию: data/books/pretrain_txt)
  --fresh           архивировать только wide_20L_16k_1024 (tokenizer не трогать)
...
if [[ ! -f "$ENV_FILE" ]]; then
    echo "[20L-WIDE] ERROR: missing preset: $ENV_FILE" >&2
    exit 1
fi
...
if [[ "${txt_count:-0}" -eq 0 ]]; then
    echo "[20L-WIDE] no .txt in $DATA_DIR — fetching ru.wikipedia starter corpus"
    python3 "$ROOT/scripts/fetch-ru-pretrain.py" --dst "$DATA_DIR"
    txt_count="$(find "$DATA_DIR" -name '*.txt' -type f 2>/dev/null | wc -l | tr -d ' ')"
fi
if [[ "${txt_count:-0}" -eq 0 ]]; then
    echo "[20L-WIDE] ERROR: no .txt files in $DATA_DIR" >&2
    exit 1
fi
...
if [[ "$DO_FRESH" -eq 1 ]]; then
    ...
    shopt -u nullglob
    echo "[20L-WIDE] --fresh: moved $moved file(s) to $CKPT_BACKUP (tokenizer untouched)"
fi
...
if [[ "$has_ckpt" -eq 1 ]]; then
    echo "[20L-WIDE] NOTE: found Adam checkpoint in $CKPT_DIR — resume"
else
    mkdir -p "$CKPT_DIR"
    if [[ -f "$CKPT_DIR/model_final.bin" ]]; then
        echo "[20L-WIDE] NOTE: model_final.bin exists, no Adam checkpoint — continue from weights, step 0"
    else
        echo "[20L-WIDE] from-scratch pretrain (no seed weights)"
    fi
fi
if [[ -f "$TOKENIZER_FILE" ]]; then
    echo "[20L-WIDE] tokenizer exists: $TOKENIZER_FILE"
else
    echo "[20L-WIDE] tokenizer missing — AllBooksTrain will train BPE on the corpus"
fi

echo ""
echo "════════════════════════════════════════════════════════════"
echo " JGPT Train 20L-wide pretrain  |  $(date '+%Y-%m-%d %H:%M:%S')"
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
    ...
    if [[ -f "$ROOT/build/libjgpt_cuda.so" ]]; then
        ...
        echo "[20L-WIDE] ERROR: --no-build but no lib in build/" >&2
        exit 1
    fi
fi
