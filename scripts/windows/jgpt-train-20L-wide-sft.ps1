...
#
# jgpt-train-20L-wide-sft.ps1 - SFT after 20L-wide pretrain. Fresh Adam.
# Same geometry. New dir: checkpoints\wide_20L_sft
# Does NOT touch 37L or the pretrain dir (except reading model_best/final).
#
# Usage:
#   .\scripts\windows\jgpt-train-20L-wide-sft.cmd --no-build
#
# Chat (after this run):
...
$LogFile = Join-Path $Root "training_20L_wide_sft.log"
$EnvFile = Join-Path $Root "env\20L-wide-sft.env"
$CkptDir = Join-Path $Root "checkpoints\wide_20L_sft"
$CkptBackup = Join-Path $Root "checkpoints\wide_20L_sft_prev_backup"
$TokenizerFile = Join-Path $Root "checkpoints\tokenizer_wide_16k.bin"
$SrcBest = Join-Path $Root "checkpoints\wide_20L_16k_1024\model_best.bin"
$SrcFinal = Join-Path $Root "checkpoints\wide_20L_16k_1024\model_final.bin"
...
function Show-Usage {
    Write-Host @"
Usage: .\scripts\windows\jgpt-train-20L-wide-sft.cmd [OPTIONS]

SFT after 20L-wide pretrain (one dialog per window, role tokens).
Data: data\sft\exam (scripts\sft-make-exam.py). Optional: --data-dir data\sft\short
Preset: env\20L-wide-sft.env
Checkpoints: checkpoints\wide_20L_sft
Seeds from checkpoints\wide_20L_16k_1024\model_best.bin (or model_final.bin), fresh Adam.

Options:
  --data-dir PATH   directory with .jsonl (default: data\sft\exam)
  --fresh           archive ONLY wide_20L_sft (tokenizer stays)
    ...
    switch ($argList[$i]) {
        "--data-dir" {
            if ($i + 1 -ge $argList.Count) {
                Write-Host "[20L-WIDE-SFT] ERROR: --data-dir requires a path" -ForegroundColor Red
                Show-Usage
                exit 1
            }
            $DataDir = $argList[$i + 1]
...
if (-not (Test-Path $EnvFile)) {
    Write-Host "[20L-WIDE-SFT] ERROR: missing preset: $EnvFile" -ForegroundColor Red
    exit 1
}
...
if ($jsonlBefore -eq 0 -and (Test-Path $makePy)) {
    Write-Host "[20L-WIDE-SFT] generating exam JSONL -> $DataDir"
    $py = $null
    ...
    if (-not $py) {
        Write-Host "[20L-WIDE-SFT] ERROR: python not found (need it to run sft-make-exam.py)" -ForegroundColor Red
        exit 1
    }
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    ...
    Write-Host "[20L-WIDE-SFT] using existing jsonl in $DataDir"
}

if (-not (Test-Path -LiteralPath $DataDir)) {
    Write-Host "[20L-WIDE-SFT] ERROR: data dir missing: $DataDir" -ForegroundColor Red
    exit 1
}
...
if ($jsonlCount -eq 0) {
    Write-Host "[20L-WIDE-SFT] ERROR: no .jsonl files in $DataDir (JGPT_SFT=1)" -ForegroundColor Red
    exit 1
}
...
if (-not $javaHome) {
    Write-Host "[20L-WIDE-SFT] ERROR: JDK 25+ not found. Set JAVA_HOME, for example:" -ForegroundColor Red
...
if (-not $userFaTile) {
    ...
    if ($cap -eq "7.5") {
        ...
        Write-Host "[20L-WIDE-SFT] GPU compute $cap (Turing): JGPT_FA_TILE_SIZE=64"
    }
}
...
if ($DoFresh) {
    ...
    Write-Host "[20L-WIDE-SFT] --fresh: moved $moved file(s) to $CkptBackup (tokenizer untouched)"
}
...
if ($hasCkpt) {
    Write-Host "[20L-WIDE-SFT] NOTE: found Adam checkpoint in $CkptDir - resume"
} else {
    ...
    if (-not (Test-Path $dstFinal)) {
        if (-not (Test-Path $SrcBest) -and -not (Test-Path $SrcFinal)) {
            Write-Host "[20L-WIDE-SFT] ERROR: missing seed weights: $SrcBest" -ForegroundColor Red
            Write-Host "  Run pretrain first: .\scripts\windows\jgpt-train-20L-wide.cmd --no-build"
            exit 1
        }
        $seed = $SrcBest
        if (-not (Test-Path $seed)) { $seed = $SrcFinal }
        Copy-Item -LiteralPath $seed -Destination $dstFinal -Force
        Write-Host "[20L-WIDE-SFT] seeded weights: $seed -> $dstFinal (fresh Adam)"
    } else {
        Write-Host "[20L-WIDE-SFT] NOTE: $dstFinal exists, no Adam checkpoint - continue from weights, step 0"
    }
}
if (-not (Test-Path $TokenizerFile)) {
    Write-Host "[20L-WIDE-SFT] ERROR: missing tokenizer: $TokenizerFile" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " JGPT Train 20L-wide SFT  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $dll)) {
        Write-Host "[20L-WIDE-SFT] ERROR: --no-build but no lib in build\ ($dll)" -ForegroundColor Red
        exit 1
    }
}
...
$mvn = Find-Mvn
if (-not $mvn) {
    Write-Host "[20L-WIDE-SFT] ERROR: Maven not found. Install it, then open a NEW PowerShell:" -ForegroundColor Red
...
Write-Host "[20L-WIDE-SFT] mvn compile..."
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
& $mvn -q compile
...
Write-Host "[20L-WIDE-SFT] starting AllBooksTrain (resume only if checkpoint_final.bin in wide_20L_sft)..."
