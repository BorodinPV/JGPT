...
#
# jgpt-train-20L-wide.ps1 - books LM pretrain, wide/short ~101M (20L / d=512 / seq 1024).
# New tokenizer (no lowercase, <user>/<assistant>). Does NOT touch 37L checkpoints.
#
# Usage:
#   .\scripts\windows\jgpt-train-20L-wide.cmd --no-build
#
# After pretrain: .\scripts\windows\jgpt-train-20L-wide-sft.cmd --no-build
$ErrorActionPreference = "Stop"

$Root = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
Set-Location $Root

$LogFile = Join-Path $Root "training_20L_wide.log"
$EnvFile = Join-Path $Root "env\20L-wide-pretrain.env"
$CkptDir = Join-Path $Root "checkpoints\wide_20L_16k_1024"
$CkptBackup = Join-Path $Root "checkpoints\wide_20L_16k_1024_prev_backup"
...
function Show-Usage {
    Write-Host @"
Usage: .\scripts\windows\jgpt-train-20L-wide.cmd [OPTIONS]

Books LM pretrain: 20 layers, d_model=512, 32 heads (d_head=16), seq=1024, vocab=16000.
Preset: env\20L-wide-pretrain.env
Checkpoints: checkpoints\wide_20L_16k_1024
...
Options:
  ...
  --fresh           archive ONLY wide_20L_16k_1024 (tokenizer stays)
    ...
    switch ($argList[$i]) {
        "--data-dir" {
            if ($i + 1 -ge $argList.Count) {
                Write-Host "[20L-WIDE] ERROR: --data-dir requires a path" -ForegroundColor Red
                Show-Usage
                exit 1
            }
            $DataDir = $argList[$i + 1]
...
if (-not (Test-Path $EnvFile)) {
    Write-Host "[20L-WIDE] ERROR: missing preset: $EnvFile" -ForegroundColor Red
    exit 1
}
...
if ($txtBefore -eq 0 -and (Test-Path $fetchPy)) {
    Write-Host "[20L-WIDE] no .txt in $DataDir - fetching ru.wikipedia starter corpus"
    $py = $null
    ...
    if (-not $py) {
        Write-Host "[20L-WIDE] ERROR: python not found (need it to run fetch-ru-pretrain.py)" -ForegroundColor Red
        exit 1
    }
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    ...
    Write-Host "[20L-WIDE] using existing txt in $DataDir"
}

if (-not (Test-Path -LiteralPath $DataDir)) {
    Write-Host "[20L-WIDE] ERROR: data dir missing: $DataDir" -ForegroundColor Red
    exit 1
}
...
if ($txtCount -eq 0) {
    Write-Host "[20L-WIDE] ERROR: no .txt files in $DataDir" -ForegroundColor Red
    exit 1
}
...
if (-not $javaHome) {
    Write-Host "[20L-WIDE] ERROR: JDK 25+ not found. Set JAVA_HOME, for example:" -ForegroundColor Red
...
if (-not $userFaTile) {
    ...
    if ($cap -eq "7.5") {
        ...
        Write-Host "[20L-WIDE] GPU compute $cap (Turing): JGPT_FA_TILE_SIZE=64"
    }
}
...
if ($DoFresh) {
    ...
    Write-Host "[20L-WIDE] --fresh: moved $moved file(s) to $CkptBackup (tokenizer untouched)"
}
...
if ($hasCkpt) {
    Write-Host "[20L-WIDE] NOTE: found Adam checkpoint in $CkptDir - resume"
} else {
    New-Item -ItemType Directory -Force -Path $CkptDir | Out-Null
    if (Test-Path (Join-Path $CkptDir "model_final.bin")) {
        Write-Host "[20L-WIDE] NOTE: model_final.bin exists, no Adam checkpoint - continue from weights, step 0"
    } else {
        Write-Host "[20L-WIDE] from-scratch pretrain (no seed weights)"
    }
}
if (Test-Path $TokenizerFile) {
    Write-Host "[20L-WIDE] tokenizer exists: $TokenizerFile"
} else {
    Write-Host "[20L-WIDE] tokenizer missing - AllBooksTrain will train BPE on the corpus"
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " JGPT Train 20L-wide pretrain  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $dll)) {
        Write-Host "[20L-WIDE] ERROR: --no-build but no lib in build\ ($dll)" -ForegroundColor Red
        exit 1
    }
}
...
$mvn = Find-Mvn
if (-not $mvn) {
    Write-Host "[20L-WIDE] ERROR: Maven not found. Install it, then open a NEW PowerShell:" -ForegroundColor Red
...
Write-Host "[20L-WIDE] mvn compile..."
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
& $mvn -q compile
...
Write-Host "[20L-WIDE] starting AllBooksTrain (resume only if checkpoint_final.bin in wide_20L_16k_1024)..."
