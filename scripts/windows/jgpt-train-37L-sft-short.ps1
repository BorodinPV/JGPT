...
#
# jgpt-train-37L-sft-short.ps1 - short Q&A finetune from 37L model_best.bin
# Same geometry (37L / 16k / 2048). New dir: checkpoints\sft_37L_short_ft
# Does NOT touch checkpoints\sft_37L_16k_2048 or retrain the tokenizer.
# Copies model_best.bin -> model_final.bin (weights only, fresh Adam).
#
# Usage:
#   .\scripts\windows\jgpt-train-37L-sft-short.ps1 --no-build
#   .\scripts\windows\jgpt-train-37L-sft-short.ps1 --fresh --no-build
#
# Stop:   Ctrl+C  (shutdown hook writes checkpoint_final.bin in the SHORT dir)
# Resume: .\scripts\windows\jgpt-train-37L-sft-short.ps1 --no-build
#
# Chat (after this run):
#   .\scripts\windows\jgpt-chat-37L-sft-short.ps1
$ErrorActionPreference = "Stop"

$Root = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
Set-Location $Root

$LogFile = Join-Path $Root "training_sft_37L_short.log"
$EnvFile = Join-Path $Root "env\37L-sft-short-ft.env"
$CkptDir = Join-Path $Root "checkpoints\sft_37L_short_ft"
$CkptBackup = Join-Path $Root "checkpoints\sft_37L_short_ft_prev_backup"
$TokenizerFile = Join-Path $Root "checkpoints\tokenizer_sft_16k.bin"
$SrcBest = Join-Path $Root "checkpoints\sft_37L_16k_2048\model_best.bin"
...
function Show-Usage {
    Write-Host @"
Usage: .\scripts\windows\jgpt-train-37L-sft-short.ps1 [OPTIONS]

Short Q&A finetune of 37L SFT: seq=2048, vocab=16000, LR=1e-4, 2 epochs.
Data: data\sft\short (built from data\sft\raw by sft-filter-short.py).
Preset: env\37L-sft-short-ft.env
Checkpoints: checkpoints\sft_37L_short_ft (NOT sft_37L_16k_2048).
Seeds weights from checkpoints\sft_37L_16k_2048\model_best.bin, fresh Adam.

Options:
  --data-dir PATH   directory with .jsonl (default: data\sft\short)
  --fresh           archive ONLY sft_37L_short_ft (tokenizer stays)
  --no-build        skip CUDA rebuild (need build\jgpt_cuda.dll)
  -h, --help        this help

Env (overrides preset, set BEFORE launching):
  JGPT_DATA_DIR           same as --data-dir
  JGPT_EPOCHS             epoch count (default 2)
  JGPT_BATCH_SIZE         if OOM: 2 (then accum=64, same 128 windows)
  JGPT_ACCUMULATION_STEPS default 32
  JGPT_LEARNING_RATE      default 0.0001
  JGPT_MAX_SEQ_LEN        if OOM: 1536
    ...
    switch ($argList[$i]) {
        "--data-dir" {
            if ($i + 1 -ge $argList.Count) {
                Write-Host "[37L-SFT-SHORT] ERROR: --data-dir requires a path" -ForegroundColor Red
                Show-Usage
                exit 1
            }
            $DataDir = $argList[$i + 1]
...
if (-not (Test-Path $EnvFile)) {
    Write-Host "[37L-SFT-SHORT] ERROR: missing preset: $EnvFile" -ForegroundColor Red
    exit 1
}
...
if ($jsonlBefore -eq 0 -and (Test-Path $filterPy)) {
    Write-Host "[37L-SFT-SHORT] filtering short JSONL from $rawDir -> $DataDir"
    $py = $null
    ...
    if (-not $py) {
        Write-Host "[37L-SFT-SHORT] ERROR: python not found (need it to run sft-filter-short.py)" -ForegroundColor Red
        exit 1
    }
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    ...
    Write-Host "[37L-SFT-SHORT] using existing jsonl in $DataDir"
}

if (-not (Test-Path -LiteralPath $DataDir)) {
    Write-Host "[37L-SFT-SHORT] ERROR: data dir missing: $DataDir" -ForegroundColor Red
    exit 1
}
...
if ($jsonlCount -eq 0) {
    Write-Host "[37L-SFT-SHORT] ERROR: no .jsonl files in $DataDir (JGPT_SFT=1)" -ForegroundColor Red
    exit 1
}
...
if (-not $javaHome) {
    Write-Host "[37L-SFT-SHORT] ERROR: JDK 25+ not found. Set JAVA_HOME, for example:" -ForegroundColor Red
...
if (-not $userFaTile) {
    ...
    if ($cap -eq "7.5") {
        ...
        Write-Host "[37L-SFT-SHORT] GPU compute $cap (Turing): JGPT_FA_TILE_SIZE=64"
    }
}
...
if ($DoFresh) {
    ...
    Write-Host "[37L-SFT-SHORT] --fresh: moved $moved file(s) to $CkptBackup (tokenizer untouched)"
}
...
if ($hasCkpt) {
    Write-Host "[37L-SFT-SHORT] NOTE: found Adam checkpoint in $CkptDir - resume"
} else {
    New-Item -ItemType Directory -Force -Path $CkptDir | Out-Null
    $dstFinal = Join-Path $CkptDir "model_final.bin"
    if (-not (Test-Path $dstFinal)) {
        if (-not (Test-Path $SrcBest)) {
            Write-Host "[37L-SFT-SHORT] ERROR: missing seed weights: $SrcBest" -ForegroundColor Red
            Write-Host "  Train 37L SFT first, or copy model_best.bin there."
            exit 1
        }
        Copy-Item -LiteralPath $SrcBest -Destination $dstFinal -Force
        Write-Host "[37L-SFT-SHORT] seeded weights: $SrcBest -> $dstFinal (fresh Adam)"
    } else {
        Write-Host "[37L-SFT-SHORT] NOTE: $dstFinal exists, no Adam checkpoint - continue from weights, step 0"
    }
}
if (-not (Test-Path $TokenizerFile)) {
    Write-Host "[37L-SFT-SHORT] ERROR: missing tokenizer: $TokenizerFile" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "============================================================"
Write-Host " JGPT Train 37L SFT short-ft  |  $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
...
if (-not $SkipBuild) {
    ...
    if (-not (Test-Path $dll)) {
        Write-Host "[37L-SFT-SHORT] ERROR: --no-build but no lib in build\ ($dll)" -ForegroundColor Red
        exit 1
    }
}
...
$mvn = Find-Mvn
if (-not $mvn) {
    Write-Host "[37L-SFT-SHORT] ERROR: Maven not found. Install it, then open a NEW PowerShell:" -ForegroundColor Red
...
Write-Host "[37L-SFT-SHORT] mvn compile..."
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
& $mvn -q compile
...
Write-Host "[37L-SFT-SHORT] starting AllBooksTrain (resume only if checkpoint_final.bin in short_ft)..."
