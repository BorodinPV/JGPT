...
$ModelCandidates = @(
    "checkpoints\wide_28L_sft\model_best.bin",
    "checkpoints\wide_28L_sft\model_final.bin",
    "checkpoints\wide_28L_16k_1024\model_best.bin",
    "checkpoints\wide_28L_16k_1024\model_final.bin"
)
$ModelDefault = $null
...
if (-not $javaHome) {
    Write-Host "[28L-CHAT] ERROR: JDK 25+ not found. Set JAVA_HOME." -ForegroundColor Red
    exit 1
}
...
if (-not $env:JGPT_CUDA_LIB -or -not (Test-Path $env:JGPT_CUDA_LIB)) {
    Write-Host "[28L-CHAT] ERROR: no CUDA JNI. Expected $dll" -ForegroundColor Red
    exit 1
}
...
if (-not (Test-Path $tokAbs)) {
    Write-Host "[28L-CHAT] ERROR: no tokenizer: $tokAbs" -ForegroundColor Red
    exit 1
}
...
if ($mvn) {
    ...
    Write-Host "[28L-CHAT] mvn not on PATH; using target\classes + .m2 jars"
    $cp = Get-FallbackClasspath $Root
    if (-not $cp) {
        Write-Host "[28L-CHAT] ERROR: no Maven and no compiled classes/jars." -ForegroundColor Red
