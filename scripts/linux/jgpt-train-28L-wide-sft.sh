#!/usr/bin/env bash
# =============================================================
# jgpt-train-28L-wide-sft.sh — SFT after 28L-wide pretrain
# Checkpoints: checkpoints/wide_28L_sft/  Tokenizer: tokenizer_wide_16k.bin
#
# Usage:
#   ./scripts/linux/jgpt-train-28L-wide-sft.sh --no-build
# Windows: .\scripts\windows\jgpt-train-28L-wide-sft.cmd --no-build
# Chat:    ./scripts/linux/jgpt-chat.sh --boo . --layers 28 --seq-len 1024 \
#            --model checkpoints/wide_28L_sft/model_best.bin \
...
LOG_FILE="$ROOT/training_28L_wide_sft.log"
ENV_FILE="$ROOT/env/28L-wide-sft.env"
CKPT_DIR="$ROOT/checkpoints/wide_28L_sft"
CKPT_BACKUP="$ROOT/checkpoints/wide_28L_sft_prev_backup"
...
SFT after 28L-wide pretrain (one dialog per window). Data: data/sft/exam
Preset: ${ENV_FILE}
...
if [[ ! -f "$ENV_FILE" ]]; then
    echo "[28L-WIDE-SFT] ERROR: missing preset: $ENV_FILE" >&2
    exit 1
fi
...
if [[ "${jsonl_count:-0}" -eq 0 ]]; then
    echo "[28L-WIDE-SFT] ERROR: no .jsonl files in $DATA_DIR (JGPT_SFT=1)" >&2
    exit 1
fi
...
if [[ "$DO_FRESH" -eq 1 ]]; then
    ...
    shopt -u nullglob
    echo "[28L-WIDE-SFT] --fresh: moved $moved file(s) to $CKPT_BACKUP (tokenizer untouched)"
fi
...
if [[ "$has_ckpt" -eq 1 ]]; then
    echo "[28L-WIDE-SFT] NOTE: found Adam checkpoint in $CKPT_DIR — resume"
else
    mkdir -p "$CKPT_DIR"
    if [[ ! -f "$CKPT_DIR/model_final.bin" ]]; then
        if [[ ! -f "$SRC_BEST" && ! -f "$SRC_FINAL" ]]; then
            echo "[28L-WIDE-SFT] ERROR: missing seed weights: $SRC_BEST" >&2
            echo "  Run pretrain first: ./scripts/linux/jgpt-train-28L-wide.sh --no-build" >&2
            exit 1
        fi
        seed="$SRC_BEST"
        [[ -f "$seed" ]] || seed="$SRC_FINAL"
        cp "$seed" "$CKPT_DIR/model_final.bin"
        echo "[28L-WIDE-SFT] seeded weights: $seed -> $CKPT_DIR/model_final.bin (fresh Adam)"
    else
        echo "[28L-WIDE-SFT] NOTE: model_final.bin exists, no Adam checkpoint — continue from weights, step 0"
    fi
fi
if [[ ! -f "$TOKENIZER_FILE" ]]; then
    echo "[28L-WIDE-SFT] ERROR: missing tokenizer: $TOKENIZER_FILE" >&2
    exit 1
fi

echo ""
echo "════════════════════════════════════════════════════════════"
echo " JGPT Train 28L-wide SFT  |  $(date '+%Y-%m-%d %H:%M:%S')"
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
    ...
    if [[ -f "$ROOT/build/libjgpt_cuda.so" ]]; then
        ...
        echo "[28L-WIDE-SFT] ERROR: --no-build but no lib in build/" >&2
        exit 1
    fi
fi
